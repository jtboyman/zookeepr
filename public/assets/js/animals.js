...
const getAnimals = (formData = {}) => {
  ...
  fetch(queryUrl)
  .then((response) =>{
    if (!response.ok) { //if not ok give error
      return alert('Error: ' + response.statusText);
    }
    return response.json(); //if ok return response parsed into readable JSON
  })
  .then(animalData => {
    console.log(animalData);
    printResults(animalData); //generates the cards for each animal from JSON data
  });
